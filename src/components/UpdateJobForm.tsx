...
const UpdateJobForm = ({ job: initialJob, onClose }: UpdateJobFormProps) => {
  const t = useI18n();
  ...
  // Récupérer les statuts lors du montage du composant
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validateJob(job)) {
      dispatch(updateJobInDatabase(job));
      onClose?.();
    }
  };
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} role="form" className="flex flex-col gap-3">
        ...
        <Button className="mt-4" type="submit" data-testid="update-job-button">
          {t("updateJob")}
        </Button>
      </form>
    </div>
